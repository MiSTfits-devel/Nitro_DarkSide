@ M5 heavy scene, ARM9 side: extended palettes + alpha blending on top of
@ the arm9_2d.s coverage. Linked at 0x02000000 (see build_nds_2dh.sh).
@
@   VRAMCNT  A -> BG, B -> OBJ; E/F start as LCDC so the CPU can write the
@            ext palettes (0x06880000 / 0x06890000), then remap E -> BG
@            ext pal slots 0-3 (MST=4) and F -> OBJ ext pal (MST=5)
@   mode 1:  BG0 text 4bpp std pal, pri 0, transparent holes, BLD 1st
@            BG1 text 256-color EXT PAL (slot 1, per-tile palettes), pri 1
@            BG3 affine 8bpp std pal, pri 3 (shows through BG0+BG1 holes)
@   OBJ:     16x16 4bpp quadrants (blend 1st target over BG1/BG3/BD)
@            8x8 semi-transparent (gfx mode 1) over BG1
@            16x16 256-color OBJ EXT PAL slot 5
@            16x16 clipped at the right edge
@   BLDCNT = 0x2A51 (alpha; 1st BG0+OBJ, 2nd BG1+BG3+backdrop), EVA/EVB 9/7
@
...
22:str  r1, [r0], #4
   ...
   bne  22b
   ldr  r0, =0x07000000
   @ spr0: 16x16 4bpp at (40,30), tiles 2, pal 0 (blend 1st target)
   ldr  r1, =0x4028001E
   str  r1, [r0]
   ldr  r1, =0x00000002
   str  r1, [r0, #4]
...
@ ==== BG control + affine + blending + display on ====
   ldr  r0, =0x0400            @ BG0: pri 0, char 0, screen 4, 4bpp
   strh r0, [r11, #0x08]
   ...
   strh r0, [r11, #0x52]
   ldr  r0, =0xC0011B11        @ mode 1, BG0+BG1+BG3+OBJ, 1D, ext pals, on
   str  r0, [r11]
