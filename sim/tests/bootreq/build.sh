#!/bin/sh
# Build the bootreq ROM: a purpose-built dual-CPU .nds that answers "does an ARM9
...
#
# Emits nds_bootreq.nds (for melonds_fbdump, as an oracle) and ../nds_bootreq.hex
...
"$NDSTOOL" -c nds_bootreq.nds -9 arm9.elf -7 arm7.elf \
   -g BREQ 01 "MISTFITS-BQ" 1
"$NDSTOOL" -i nds_bootreq.nds | head -12

python3 - <<'EOF'
img = open("nds_bootreq.nds", "rb").read()
img += b"\0" * ((4 - len(img) % 4) % 4)
with open("../nds_bootreq.hex", "w") as f:
    for i in range(0, len(img), 4):
        f.write(f"{int.from_bytes(img[i:i+4], 'little'):08x}\n")
print(f"nds_bootreq.hex: {len(img)//4} words")
EOF
