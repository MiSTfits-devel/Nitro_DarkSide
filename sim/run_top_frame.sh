...
VRAMOPS="${VRAMOPS:-0}"
# >0 with VRAMOPS: per-line cost profile of the first 16 lines of that frame
LINEPROF_FRAME="${LINEPROF_FRAME:--1}"
GPUFAST="${GPUFAST:-0}"
...
nvc -H "$NVCHEAP" -L "$WORK" --work="$WORK/work" -e "$NVCOPT" tb_top_frame \
   ...
   -gDUMPFILE="$DUMPFILE" -gDUMPFILE_B="$DUMPFILE_B" -gDUMP_START_FRAME="$DUMP_START_FRAME" -gDIRECT="$DIRECT" -gFWBOOT="$FWBOOT" -gHEARTBEAT_MS="$HEARTBEAT_MS" -gVRAMOPS="$VRAMOPS" -gLINEPROF_FRAME="$LINEPROF_FRAME" -gGPUFAST="$GPUFAST" -gSOUND="$SOUND" -gVRSRV_LAT="$VRSRV_LAT" -gVRSRV_ONE="$VRSRV_ONE" -gVRSRV_OUT="$VRSRV_OUT" -gVRSRV_GAP="$VRSRV_GAP" -gISLAND="$ISLAND" -gISLAND_HALF_PS="$ISLAND_HALF_PS" \
